function [K] =  FedGTF_EF_prox(client,nd,K,prox,dim,rank,subs,vals,wgts,isLogit,isCyclic,params,lr)
        ...
        for iter = 1:epciters
            ...
            if (rd_n == 1)
                for k = 1:K
                    ...
                    client(k).P_rd_n = gamma * client(k).G_rd_n + client(k).EF{rd_n};
                    ...
                    client(k).U{rd_n} = proximal_l1( client(k).U{rd_n}- client(k).Delta_rd_n,prox/gamma);
                ...
                for k = 1:K
                    client(k).U{rd_n} = proximal_l1( client(k).U{rd_n}-  sumDelta, prox/gamma);
                end
            end
        end
		time = cputime-start;
...
function [Ud_new] = proximal_l1(Ud_old,lambda)
    Ud_new =max(0,Ud_old-lambda)+min(0,Ud_old+lambda);
end