function [G] =  FedGTF_EF(client,nd,K,dim,rank,subs,vals,wgts,isLogit,isCyclic,params,lr)
        ...
        for iter = 1:epciters
            ...
            if (rd_n == 1)
                for k = 1:K
                    ...
                    client(k).U{rd_n} = client(k).U{rd_n} - client(k).Delta_rd_n;
                end		
            else
                for k = 1:K
                    client(k).subs = sample_mode_n(nsamplsq, client(k).info.size, rd_n);
                ...
                for k = 1:K
                    client(k).U{rd_n} = client(k).U{rd_n} - sumDelta;
                end
            end
        end
		time = cputime-start;
